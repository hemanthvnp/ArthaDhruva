...
/**
 * Applies a fitted isotonic regression (piecewise-linear interpolation over the fitted
 * breakpoints) to convert a model's raw, badly-overpredicted output into a calibrated
 * probability -- see default_risk_model.ipynb's calibration check: raw output overpredicts by
 * ~17.5x on average; the isotonic fix reduces that to ~1.0x with no change to ranking (AUC),
 * since it's a monotonic transform.
 *
 * Generic ML infrastructure, not specific to any one scoring domain -- shared by {@code score}
 * and {@code earlywarning}, both of which calibrate a LightGBM model's raw output the same way.
 */
